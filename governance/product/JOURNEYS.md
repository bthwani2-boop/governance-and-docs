...
FIELD ADMISSION/ELIGIBILITY → IDENTITY RESOLUTION + `field` ROLE ADMISSION → CANONICAL Field actor binding → authorized Field-originated joining work → OWNER REVIEW; then the Partner path remains PROSPECTIVE PARTNER → JOINING CASE WITH FIRST-STORE SERVICE CITY + PRIMARY COMMERCE VERTICAL → REQUIRED BUSINESS/FIRST-STORE DATA → IDENTITY RESOLUTION + partner ROLE ADMISSION WHEN ELIGIBLE → CANONICAL actor_id BINDING → OWNER REVIEW/CORRECTION → STORE READINESS → STORE PUBLICATION → CENTRAL CATALOG PRODUCT/VARIANT SELECTION → STORE OFFER PRICE / QUANTITY / AVAILABILITY / PUBLICATION → CUSTOMER-VISIBLE STOREFRONT READBACK.
...
CUSTOMER ADDRESS/SERVICEABILITY → CART/CHECKOUT → ORDER CREATION → PARTNER ACCEPT/PREPARE → READY FOR DISPATCH → CAPTAIN OFFER/ASSIGNMENT → STORE↔CAPTAIN HANDOFF → PICKED UP → IN DELIVERY → DELIVERED OR GOVERNED DELIVERY FAILURE → (when failed) OPERATOR-OWNED SAME-CAPTAIN RECOVERY → IN DELIVERY → DELIVERED → CANONICAL CROSS-SURFACE READBACK.
...
DSH CAPTAIN ADMISSION / STANDING OPERATIONAL ELIGIBILITY → AUTHORIZED IDENTITY REQUEST → IDENTITY ACTOR RESOLUTION + `captain` ROLE ADMISSION → ACTIVATION / ENROLLMENT → DSH AVAILABILITY → DISPATCH OFFER → ACCEPT/DECLINE/TIMEOUT → ASSIGNMENT → STORE HANDOFF → PICKUP → FINAL-MILE DELIVERY → DELIVERY RESULT → CANONICAL READBACK.
