...
PROSPECTIVE PARTNER → JOINING CASE → REQUIRED BUSINESS/FIRST-STORE DATA → IDENTITY RESOLUTION + partner ROLE ADMISSION WHEN ELIGIBLE → CANONICAL actor_id BINDING → OWNER REVIEW/CORRECTION → STORE READINESS → STORE PUBLICATION → CENTRAL PRODUCT SELECTION → STORE ASSORTMENT PRICE / AVAILABILITY / PUBLICATION → CUSTOMER-VISIBLE READBACK.
