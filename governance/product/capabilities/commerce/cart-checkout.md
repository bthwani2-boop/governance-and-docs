...
**Problem.** Cart mutation, checkout validation and WLT handoff can diverge when item price, serviceability, address, delivery eligibility, quote or idempotency state is recomputed independently across client/order/payment layers.
...
**Required outcome.** A customer can build one owned Store cart, receive canonical current pricing/address/serviceability/BThwani-delivery validation, and progress through one checkout intent to a confirmed/eligible handoff for Order Creation without duplicating money or operational truth.
...
- standard checkout uses the one canonical BThwani delivery path and consumes current DSH serviceability/zone/SLA/capacity evidence;
- the client supplies delivery-address intent but does not choose an execution lane;
- non-empty checkout cannot be financially ready without the required current WLT quote/payment evidence;
- checkout intent snapshots the governed address/serviceability/delivery/commercial evidence needed for downstream Order Creation;
...
- no client-controlled field selects a different fulfillment execution path;
- no dormant alternate execution branch, hidden selector or compatibility enum is retained in cart/checkout semantics;
...
**Failure/recovery.** stale version, item/assortment invalidation, address/serviceability denial, zone/capacity pause, quote unavailable/stale, WLT timeout/unknown, idempotency conflict or checkout expiry; reread canonical owners, preserve the original logical operation identity and reconcile ambiguous WLT outcomes before retry.

**Acceptance expectations.** cart totals are reproducible from authoritative evidence; checkout readiness reports exact blocking reasons; serviceability always resolves against the canonical BThwani delivery path; canonical readback survives retry/restart; confirmed checkout carries the immutable evidence required by Order Creation; no duplicate order/payment effect is possible from a replayed intent.

**Actor responsibility envelope**
- `customer` — mutates only the owned cart, supplies an owned delivery-address intent and confirms checkout; forbidden: supply authoritative money/eligibility, choose an alternate execution path, select another actor/cart or bypass blockers.
- `DSH cart/checkout system` — validates owner evidence, versions/idempotency, snapshots checkout evidence and orchestrates WLT references; forbidden: post ledger truth, fabricate WLT success or manufacture a second fulfillment path.
...
**Surface semantics**
- `app-client` — required; states include empty, active, validating, ready, blocked, confirming, confirmed, expired, conflict, offline, partial and error; actions include add/update/remove, choose/change owned delivery address, refresh/requote, confirm and resume.
...
- technical presentation binding — implementation evidence only; maps contract state and canonical readback without local commerce truth or execution-path selection.
